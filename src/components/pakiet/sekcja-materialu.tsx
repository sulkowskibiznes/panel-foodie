...
export function SekcjaMaterialu({ material, podglad, watek, narzedzia }: { material: MaterialDto; podglad: ReactNode; watek: ReactNode; /** Zespół: przyciski „Edytuj", „Podmień" nad podglądem. */ narzedzia?: ReactNode }) {
  ...
  return (
    <section id={`material-${material.id}`} data-material={material.id} data-typ={material.typ} className="rounded-xl bg-white p-4 shadow-miekki sm:p-6">
      ...
      {narzedzia ? <div className="mb-3">{narzedzia}</div> : null}
      {podglad}
