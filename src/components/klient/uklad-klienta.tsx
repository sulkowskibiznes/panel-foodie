 ...
 */
export function UkladKlienta({ token, nazwaKlienta, etykietaOsoby, children }: { token: string; nazwaKlienta: string; etykietaOsoby: string; children: ReactNode }) {
  const baza = `/p/${token}`;
  const pozycje = [
    { href: `${baza}/start`, etykieta: copy.nawigacja.start, Ikona: Home, aktywna: true },
    { href: `${baza}/materialy`, etykieta: copy.nawigacja.materialy, Ikona: Inbox, aktywna: false },
    ...
    <div className="flex min-h-full flex-1 flex-col bg-szary-050 lg:flex-row">
      <aside className="hidden w-72 shrink-0 flex-col border-r border-szary-100 bg-white lg:flex">
        ...
        <nav aria-label={copy.marka.panel} className="flex-1 space-y-1 px-3">
          {pozycje.map(({ href, etykieta, Ikona, aktywna }) =>
            aktywna ? (
              <Link key={href} href={href} className="flex items-center gap-3 rounded-lg bg-fiolet-050 px-3 py-2.5 text-sm font-medium text-fiolet-700">
      ...
      <div className="flex flex-1 flex-col">
        ...
        <nav aria-label={copy.marka.panel} className="fixed inset-x-0 bottom-0 z-40 grid grid-cols-4 border-t border-szary-100 bg-white lg:hidden">
          {pozycje.slice(0, 3).map(({ href, etykieta, Ikona, aktywna }) =>
            aktywna ? (
              <Link key={href} href={href} className="flex flex-col items-center gap-1 px-2 py-2.5 text-[11px] font-medium text-fiolet-700">
                <Ikona className="size-5" aria-hidden />
